...
int main(int, char **)
{
...

	{
		...
		CustomContainer<int> std_alloc_list;
		for (int i = 0; i < 10; ++i)
			std_alloc_list.push_back(i);

		for (auto i : std_alloc_list)
			std::cout << i << " ";
		std::cout << std::endl; 

		// custom container with custom allocator
		CustomContainer<int, MemoryAllocator<int, 10>> cust_alloc_list;
		for (int i = 0; i < 10; ++i)
			cust_alloc_list.push_back(i);

		for (auto i : cust_alloc_list)
			std::cout << i << " ";
		std::cout << std::endl;
	}
